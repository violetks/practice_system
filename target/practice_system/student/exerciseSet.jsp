...
<%
    ...
    List<Record> recordList = recordDao.getExciseResult(student.getSid());
%>
<!DOCTYPE html>
<html>
<head>
...
<div>
    ...
    <table width="60%" border="0" align="center" id="questionTable">
        ...
        <tr align="center">
            ...
            <td width="207"><%=record.getQuestion().getqName() %>
            </td>
            <td width="157"><%if (record.getQuestion().getqType() == 0) {%>选择题
                <% } else if (record.getQuestion().getqType() == 2) {%>填空题
                <% } else if (record.getQuestion().getqType() == 4) {%>编程题
                <% } else {%>其他<%} %>
            </td>
            <td width="157"><%if (record.getQuestion().getqLevel() == 0) {%>较易
                <% } else if (record.getQuestion().getqLevel() == 1) {%>容易
                <% } else if (record.getQuestion().getqLevel() == 2) {%>难
                <% } else {%>较难<%} %>
            </td>
            <td width="157"><%=record.getSubmitTime() %>
            </td>
        </tr>
        <% } %>
    </table>
